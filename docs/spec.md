...
`TRANSPORT` 環境変数で起動モードを切り替える。デフォルトは `stdio`。

| 値 | 用途 |
|---|---|
| `stdio`（デフォルト） | Claude Code からローカル利用（Phase 1） |
| `http` | Railway デプロイ → claude.ai / スマホから利用（Phase 2） |

### 6.2 エントリポイント構成

```
src/
├── index.ts        # TRANSPORT 環境変数で stdio / http を分岐
└── mcp-server.ts   # createServer() ファクトリ（全ツール登録、両モード共用）
```
...
### 6.4 Streamable HTTP モード

**HTTP エンドポイント（`/mcp`）:**

| メソッド | 役割 |
|---|---|
| `POST` | 初期化（`mcp-session-id` ヘッダーなし）またはメッセージ送信（あり） |
| `GET` | SSE ストリーム確立（サーバー → クライアント通知） |
| `DELETE` | セッション終了 |

**セッション管理:**

- `StreamableHTTPServerTransport` は 1 インスタンス = 1 セッション
- `Map<sessionId, transport>` でサーバープロセス内に保持
- `onsessioninitialized` / `onsessionclosed` コールバックで Map を更新
...
- `createMcpExpressApp({ host: "0.0.0.0" })` を使用（Railway は全インターフェースにバインド）
- `express.json()` は `createMcpExpressApp` が自動で適用
- `0.0.0.0` バインド時は DNS rebinding protection が無効（警告のみ）

**認証:**

- Phase 2 は **authless**（claude.ai カスタムコネクターは OAuth か authless の2択）
- 将来的な保護はインフラ層（Cloudflare Access / IP 制限等）で対応

**環境変数:**

```env
TRANSPORT=http
PORT=3000  # Railway は自動設定
```
...
```bash
node dist/index.js
# TRANSPORT=http は Railway の環境変数として設定
```
...
furikaeri-mcp/
├── src/
│   ├── index.ts                  # エントリポイント（stdio / http 分岐）
│   ├── mcp-server.ts             # createServer() ファクトリ
│   ├── tools/
